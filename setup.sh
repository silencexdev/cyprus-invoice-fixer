...
if ! command -v docker &>/dev/null; then
  ...
  if [[ "$OS" == "mac" ]]; then
    ...
    echo -e "${YELLOW}  NOTE: Log out and back in (or run: newgrp docker) for group changes to take effect.${NC}"
    # Try to start docker daemon
    sudo systemctl start docker 2>/dev/null || true
  elif [[ "$OS" == "windows" ]]; then
    echo -e "${YELLOW}  Please install Docker Desktop for Windows: https://docs.docker.com/desktop/install/windows-install/${NC}"
    exit 1
  fi
else
...
if ! docker info &>/dev/null; then
  ...
  if [[ "$OS" == "mac" || "$OS" == "windows" ]]; then
    ...
    for i in $(seq 1 30); do
      sleep 1; docker info &>/dev/null && break
      echo -n "."
    done
    echo ""
  else
...
echo "  2) OpenAI — GPT-4o-mini (needs API key, ~\$0.001/invoice)"
...
echo -e "\n${BLUE}── Stripe (optional — leave blank to skip) ──${NC}"
read -rp "Stripe secret key (or press Enter to skip): " STRIPE_SECRET_KEY
read -rp "Stripe webhook secret (or press Enter to skip): " STRIPE_WEBHOOK_SECRET
read -rp "Stripe price ID (or press Enter to skip): " STRIPE_PRICE_ID
...
launch_app() {
  ...
  if [ "${AI_PROVIDER:-openai}" = "ollama" ]; then
    ...
    echo -e "${YELLOW}⚡ Pulling Ollama model ${OLLAMA_MODEL:-llama3} (this may take a few minutes)...${NC}"
    sleep 8
